...
async function runAgentLoop({ model, url, systemPrompt, prompt, yolo, plan }) {
  ...
  const activeTools = plan 
    ? tools.schemas.filter(schema => ['read', 'readLines', 'grep', 'ls', 'tree'].includes(schema.function.name))
    : tools.schemas;
  ...
  while (true) {
    ...
    if (!message.tool_calls || message.tool_calls.length === 0) {
      ...
      console.log(`[Token Usage] Final Context: ${response.usage ? response.usage.total_tokens : 0} tokens | Session Cumulative: ${totalTokens} tokens`);
      break;
    }
    ...
    for (const toolCall of message.tool_calls) {
      ...
      const argsStr = Object.keys(displayArgs).length > 0 ? ` with args: ${JSON.stringify(displayArgs)}` : '';
