...
test("TSC Output Filtering and Formatting", async (t) => {
  ...
  await t.test("filters out errors from other files", async () => {
    ...
    await fs.writeFile(
      otherFile,
      'export const a: number = ("string" as any);',
    );
    ...
    assert.strictEqual(
      result,
      "Syntax check passed.",
      "Should pass syntax check even if dependency has errors",
    );
  });
  ...
  await t.test("only shows errors from the current file", async () => {
    ...
    await fs.writeFile(
      mainFileWithError,
      "import { a } from './other_with_error.js';\nconst b: number = 1;\nconst c: string = 1;",
    );

    const result = await checkSyntax(mainFileWithError, tmpDir);
    ...
    assert.ok(
      result.includes("const c: string = 1;"),
      "Should contain error context from main file",
    );
    // Should NOT contain the path to the other file (since it's filtered)
    assert.ok(
      !result.includes("other_with_error.ts"),
      "Should NOT contain error from dependency",
    );
  });
  ...
  await t.test("formats TSC diagnostic with code context", async () => {
    ...
    const result = await checkSyntax(errorFile, tmpDir);
